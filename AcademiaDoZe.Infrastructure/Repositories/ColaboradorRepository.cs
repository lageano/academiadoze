...
public class ColaboradorRepository : BaseRepository, IColaboradorRepository
{
    ...
    public static Colaborador Map(DbDataReader reader, string nomeColumn = "nome")
    {
        ...
        try
        {
            id = reader.GetInt32Value("id_colaborador");
        ...
        {
            throw new InfrastructureException("ERRO_MAPEAMENTO_COLABORADOR", $"Erro ao mapear dados do colaborador ID {id}: {ex.Message}", ex);
        }
    }
    ...
    private static void AdicionarParametrosComuns(DbCommand command, Colaborador entity)
    {
