...
public class MatriculaRepository : BaseRepository, IMatriculaRepository
{
    ...
    public static Matricula Map(DbDataReader reader)
    {
        ...
        {
            throw new InfrastructureException("ERRO_MAPEAMENTO_MATRICULA", $"Erro ao mapear dados da matrícula ID {id}: {ex.Message}", ex);
        }
    }
    ...
    private static void AdicionarParametrosComuns(DbCommand command, Matricula entity)
    {
        ...
        command.AddParameter("@RestricaoMedica", (int)entity.Restricoes, DbType.Int32);
    ...
    public async Task<IEnumerable<Matricula>> ObterAtivas(int alunoId = 0, CancellationToken cancellationToken = default)
    {
        try
        {
            string filtroAluno = alunoId > 0 ? "AND m.aluno_id = @AlunoId" : string.Empty;
            string query = $"{BaseSelectQuery} WHERE m.data_fim >= {GetCurrentDateFunction()} {filtroAluno} ORDER BY m.data_fim ASC";
