...
public class AlunoRepository : BaseRepository, IAlunoRepository
{
    ...
    public static Aluno Map(DbDataReader reader, string nomeColumn = "nome")
    {
        ...
        try
        {
            id = reader.GetInt32Value("id_aluno");
            ...
            string email = reader.GetNullableString("email");
        ...
        {
            throw new InfrastructureException("ERRO_MAPEAMENTO_ALUNO", $"Erro ao mapear dados do aluno ID {id}: {ex.Message}", ex);
        }
    }
    ...
    private static void AdicionarParametrosComuns(DbCommand command, Aluno entity)
    {
        ...
        // e-mail é opcional para o aluno; a coluna não aceita nulo, então gravamos texto vazio
        command.AddParameter("@Email", entity.Email?.Valor ?? string.Empty, DbType.String);
