...
public class AlunoService : IAlunoService
{
    ...
    public async Task<AlunoDto> AdicionarAsync(AlunoDto alunoDto, CancellationToken cancellationToken = default)
    {
        ...
        var logradouro = await ObterLogradouroAsync(alunoDto.Endereco?.Id ?? 0, cancellationToken);
    ...
    // O endereço do aluno exige a entidade Logradouro completa, então ela é resolvida pelo repositório.
    private async Task<Domain.Entities.Logradouro> ObterLogradouroAsync(int logradouroId, CancellationToken cancellationToken)
    {
        if (logradouroId <= 0)
            throw new ArgumentException("Logradouro/Endereço é obrigatório para o aluno.", nameof(logradouroId));
